...
def _normalize_api_template(row: dict[str, Any]) -> dict[str, Any]:
    ...
    file_path_source = row.get("file_path_source") or file_path
    # template_url is used for render API input; must come from file_path.
    template_url = row.get("template_url") or build_template_url(str(file_path))
    # selected_template_url is used for redis push display/debug.
    selected_template_url = build_template_url(str(file_path_source))
    ...
    return {
        ...
        "selected_template_url": selected_template_url,
